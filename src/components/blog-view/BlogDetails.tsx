...
interface Article {
  ...
  attributes: {
    ...
    blocks: Array<{ __component: string; body?: string; url?: string }>;
    category: {
      data: {
        attributes: {
          name: string;
...
export default function BlogDetails({ data }: { data: Article }) {
  const { theme } = useTheme();
  const { title, description, publishedAt, cover, authorsBio, blocks } =
    data.attributes;
            ...
            >
              ...
              >
                <div className="flex flex-row justify-center items-center gap-3">
                  <Image
                    src={authorImgUrl || '/images/default-avatar.png'}
                    alt="fake details"
                    height={100}
                    width={100}
                  ...
                  <div className="flex flex-row gap-2">
                    <span className="opacity-50"> par </span>
                    <span> {authorsBio?.data.attributes.name} </span>
                  </div>
                </div>
